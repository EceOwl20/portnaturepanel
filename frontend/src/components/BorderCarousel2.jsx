...
const BorderCarousel2 = ({images}) => {
  ...
  const scrollPrev = useCallback(() => {
    if (emblaApi) emblaApi.scrollNext(); // Ters yöne git
    setCurr((curr) => (curr === 0 ? images.length - 1 : curr - 1));
  }, [emblaApi, curr, images.length]);

  const scrollNext = useCallback(() => {
    if (emblaApi) emblaApi.scrollPrev(); // Ters yöne git
  ...
  return (
    <div className="flex flex-col lg:w-11/12 xl:w-full h-auto items-end justify-center my-0" dir="rtl" >
      <div className="overflow-hidden relative xl:h-[698px] w-[90%] lg:w-[75%]" ref={emblaRef}>
        <div className="flex grid-flow-col">
          {images.map((image, index) => (
            <div
              className="flex-[0_0_auto] mx-1 md:mx-2 sm:w-[calc(50%-1rem)]  lg:mx-4 xl:mx-2 lg:w-[calc(33.3%-1rem)] xl:w-[calc(33.3%-0.5rem)]  w-11/12 flex justify-center relative group"
              key={index}
            >
...
export default BorderCarousel2;
